...
const getCoverageNotApprovedByMonth = asyncHandler(async (req, res) => {
  ...
  const newMonth = currentMonth < 10 ? "0" + currentMonth : currentMonth.toString();
  ...
  const startDate = new Date(`${currentYear}-${newMonth}-01T00:00:00.000Z`);
  const lastDayOfMonth = new Date(currentYear, newMonth, 0).getDate();
  const endDate = new Date(
    `${currentYear}-${newMonth}-${lastDayOfMonth}T23:59:59.999Z`
  );
